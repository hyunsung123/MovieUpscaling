...
**Upscale**과 **Output**은 캡처 중에도 변경할 수 있습니다. 다음 캡처 프레임부터 적용하고 캡처 세션은 재시작하지 않습니다. 정지 영상에서 WGC가 새 프레임을 보내지 않으면 `pending next frame`으로 표시합니다. **Sharpen 슬라이더/F3 변경은 예외로 마지막 GPU 프레임을 한 번 다시 그려 즉시 적용합니다**. 이때 대기 중이던 출력/크롭 설정도 함께 적용됩니다. 반복 렌더 루프는 없으며 설정은 Stop/Start 동안 유지되고 앱을 종료하면 기본값으로 돌아갑니다.
...
- 프레임 도착과 명시적인 Sharpen/F3 변경에 렌더링합니다. 1초 UI 타이머는 통계만 갱신합니다. 정적인 창은 프레임 도착이 멈출 수 있으며, 강도 변경 시에는 마지막 GPU 입력을 한 번 사용합니다. 이 재표시는 Render FPS에만 포함되고 Capture FPS를 늘리지 않습니다.
...
RX 6700 XT / Release에서 기존 6개와 새 4개를 포함한 **CTest 10개 모두 통과**했습니다. 새 검증은 다음과 같습니다.
